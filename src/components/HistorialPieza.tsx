...
export function HistorialPieza({ eventos }: { eventos: EventoHistorialPieza[] }) {
    ...
    <div className="flex flex-col gap-2 rounded-md border border-border p-4 lg:gap-3 lg:p-5">
      ...
      <ul className="flex flex-col">
        {eventos.map((evento, index) => {
          ...
          return (
            <li key={`${evento.estado}-${evento.created_at}`} className="flex gap-3">
              ...
              <div className={`flex flex-1 items-baseline justify-between gap-3 ${esUltimo ? "pb-0" : "pb-4"}`}>
                <span className={`text-body ${esUltimo ? "text-text-primary" : "text-text-secondary"}`}>
