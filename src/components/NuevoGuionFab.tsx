 ...
 *  flotante, `variant="fab"`, el `BottomNav` entero lleva `md:hidden`) y
 *  dentro de `Sidebar` (fila de nav, `variant="sidebar"`, visible desde
 *  `md:` — icono solo en el riel de tablet, icono + texto desde `lg:`) —
 *  sin la segunda, el atajo de crear guion no existía en absoluto en tablet
 *  ni escritorio. Cada instancia tiene su propio estado `abierto`, sin
      ...
      {abierto &&
        createPortal(
          ...
          `}</style>
            ...
            />
            <div
              className="absolute inset-x-0 bottom-0 flex flex-col gap-4 rounded-t-md bg-bg-primary p-5 pb-[calc(1.5rem+env(safe-area-inset-bottom))] md:inset-x-auto md:right-6 md:bottom-6 md:w-80 md:rounded-md"
                ...
                {(plataformas ?? []).map((plataforma) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        router.push(`/contenido/${plataforma}/videos/nueva`)
                      }
