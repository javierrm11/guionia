...
function escenasDeEstructura(
  estructuras: Estructura[],
  estructuraId: string,
): EscenaEditable[] {
  const estructura = estructuras.find((e) => e.id === estructuraId);
  if (!estructura) return [];
  return [...estructura.estructura_escenas]
...
export function GuionForm({
  ...
  estructuras: Estructura[];
  /** Estructura preseleccionada (ver `lib/estructuraSugerida.ts`) — la que
   *  más se ha usado hasta ahora en esta plataforma. Sigue siendo un
   *  desplegable normal, editable. */
  estructuraSugeridaId?: string;
  /** Día/hora con mejores vistas históricas en YouTube (ver
   *  `lib/youtube/mejorMomentoPublicacion.ts`) — solo se pasa cuando
   *  `plataforma === "youtube"`, es solo informativo, no cambia el valor
   *  del campo de fecha. */
  mejorMomento?: { diaSemana: number; hora: number; muestras: number } | null;
  frases: Frase[];
  fechaHoy: string;
  ...
  const [estructuraId, setEstructuraId] = useState(estructuraSugeridaId ?? "");
  const [escenas, setEscenas] = useState<EscenaEditable[]>(() =>
    estructuraSugeridaId
      ? escenasDeEstructura(estructuras, estructuraSugeridaId)
      : [],
  );
  const [nuevoTipo, setNuevoTipo] = useState<TipoEscena | "">("");
  const [nuevaDuracion, setNuevaDuracion] = useState("");
  const [textosPuntuacion, setTextosPuntuacion] = useState<
    Record<string, string>
  >({});
  // Qué frase guardada (hook/CTA) se insertó en cada escena, si alguna — se
  // manda como `escena_frase_origen_id` para poder medir después qué frases
  // funcionan mejor en los vídeos publicados. Se queda aunque el texto se
  // edite a mano después de insertarla (mide "partiste de esta frase", no
  // "el texto final es idéntico").
  const [frasesOrigen, setFrasesOrigen] = useState<Record<string, string>>({});
  const textareaRefs = useRef<Record<string, HTMLTextAreaElement | null>>({});

  const estructuraActual =
    estructuras.find((e) => e.id === estructuraId) ?? null;
  ...
  function elegirEstructura(id: string) {
    setEstructuraId(id);
    setEscenas(escenasDeEstructura(estructuras, id));
  }
