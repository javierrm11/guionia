...
function construirPrompt(input: GenerarEscenaInput): string {
  const partes: string[] = [
    "Eres un guionista experto en contenido corto para redes sociales (TikTok, YouTube Shorts).",
  ...
  if (input.pilar) {
    partes.push(
      `Pilar de contenido: ${PILAR_LABEL[input.pilar] ?? input.pilar}`,
    );
  }
  ...
  if (input.tipoEscena === "hook") {
    partes.push(
      "El hook debe enganchar en la primera frase, plantear el problema o la promesa del vídeo.",
    );
  } else if (input.tipoEscena === "cta") {
    partes.push(
      "El CTA debe invitar a seguir, comentar o guardar, en una frase corta.",
    );
  }
  ...
  if (otras.length > 0) {
    partes.push(
      "",
      "Resto del guion ya escrito, para que esta escena encaje con el resto:",
    );
...
export async function generarTextoEscena(
