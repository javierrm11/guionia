...
- **`/contenido/cuenta`**: estadísticas de cada cuenta conectada, una sección por plataforma (`Suspense` independiente cada una, para que una lenta no bloquee a la otra), usando la tarjeta compartida `StatMes` (número + flecha ↑/↓ verde/roja con el % respecto al mes anterior):
  ...
  - **TikTok** (`CuentaTiktokSection.tsx`): avatar + nombre vía `obtenerCuentaPropia` (solo `user.info.basic`, la misma que usa `/api/tiktok/callback` para conectar) y, aparte, seguidores/likes totales/vídeos vía `obtenerEstadisticasCuentaTiktok` (requiere el scope `user.info.stats`) — deliberadamente en **dos llamadas separadas**: si el scope de estadísticas todavía no está aprobado en el portal de TikTok, esa segunda llamada falla sola y solo oculta los contadores (con un aviso), sin tirar abajo el nombre/avatar ni, sobre todo, sin poder romper el flujo de conexión (`obtenerCuentaPropia` es la que usa el callback OAuth). TikTok no tiene una API de analítica agregada por fechas, así que "este mes vs. el anterior" se calcula sumando a mano las estadísticas (ya pedidas por vídeo en otras partes de la app) de tus propios vídeos publicados en cada mes — no es un dato de la API, es un cálculo local.
