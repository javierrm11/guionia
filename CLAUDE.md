...
- **`/contenido/cuenta`**: estadísticas de la cuenta de YouTube conectada (avatar + nombre del canal, suscriptores/vistas totales/vídeos, y una comparativa "este mes vs. el anterior" — vistas, comentarios, likes, suscriptores ganados — vía `obtenerComparativaMensual`, Analytics API con `dimensions=month`) y, debajo, una fila **Ajustes** que enlaza a `/configuracion`. Es la pestaña "Cuenta" de la barra inferior — sustituye a lo que antes era un acceso directo a Ajustes.
